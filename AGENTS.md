...
本目录是项目根目录。`src/` 包含入口、Provider、配置和 AI 处理代码；`.pi/skills/platypus/references/provider/` 保存各 Provider 的官方 API 文档入口；`README.md` 说明项目用法。旧版 MCP 的 `src/` 已删除；CLI 已发布到 npm；文档不得把尚未验证的能力写成已验证。

## 工作流程

开始项目任务时先阅读 `README.md` 了解现状；经用户批准修改文件后，同步更新相关文档，避免把未完成事项写成已完成。适配上游或更新文档时，按需阅读 `.pi/skills/platypus/references/provider/{provider}/advanced.md` 获取官方 API 文档地址，再查官方文档确认细节；reference 不替代官方文档。

## 重构目标

项目由 Node.js / TypeScript、ESM 实现，已从 MCP 服务器改为 CLI。npm 包名为 `@xyzensun/platypus`，可执行命令为 `platypus`；构建产物带可执行入口，本地安装可运行 `platypus search exa ...`。当前适配 Exa、Tavily、Jina、Brave、Gemini、Ollama、SearXNG；Jina 和 SearXNG 尚待真实 API 验证。
